...
export function useAsync(initialState?: AsyncState) {
  ...
  const setData = React.useCallback((data: any) => safeSetState({ status: 'resolved', data }), [safeSetState]);
  const setError = React.useCallback((error: any) => safeSetState({ status: 'rejected', error }), [safeSetState]);
